...
void gestion_monster_volcan(sfRenderWindow* window, game *ga, sfMusic *music)
{
    ...
    i += monster_damages_volcan(ga, ga->mo.s_volcano_monster1);
    i += monster_damages_volcan(ga, ga->mo.s_volcano_monster2);
    i += monster_damages_volcan(ga, ga->mo.s_volcano_monster3);
    i += monster_damages_volcan(ga, ga->mo.s_volcano_monster4);
    if (i != 0) {
        sfMusic_stop(music);
        over(window, ga);
    }
